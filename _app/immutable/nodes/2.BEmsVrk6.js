...
}`,wt=e=>Math.pow(2,Math.ceil(Math.log2(e))),Me=(e="morivis",t=1280,o="#ffffff",u="Arial, sans-serif")=>{const s=document.createElement("canvas"),i=s.getContext("2d");if(!i)throw new Error("Failed to get canvas context");i.font=`${t}px ${u}`,i.textAlign="left",i.textBaseline="top";const x=i.measureText(e),p=Math.ceil(x.width)+20,_=t+20,h=wt(p),r=wt(_);return s.width=h,s.height=r,i.clearRect(0,0,h,r),i.font=`${t}px ${u}`,i.textAlign="left",i.textBaseline="top",i.fillStyle=o,i.fillText(e,10,10),i.getImageData(0,0,s.width,s.height),s},G={time:{value:0},uColor:{value:new dt("rgb(252, 252, 252)")},uColor2:{value:new dt("rgb(0, 194, 36)")},resolution:{value:new g(window.innerWidth,window.innerHeight)}},K={screenCenter:{value:new g(.5,.5)},resolution:{value:new g(window.innerWidth,window.innerHeight)},screenTexture:{value:null},uTexture:{value:new $t(Me())},uTextureResolution:{value:new g(1e3,750)}},Ce=new bt({uniforms:G,vertexShader:Pe,fragmentShader:Ee,transparent:!0}),Te=async(e,t=.1)=>{const u=await(await fetch(e)).blob(),s=new Image;s.src=URL.createObjectURL(u),await new Promise((r,a)=>{s.onload=r,s.onerror=a});const i=document.createElement("canvas");i.width=s.width,i.height=s.height;const x=i.getContext("2d");if(!x)throw new Error("Failed to get canvas context");x.drawImage(s,0,0);const p=x.getImageData(0,0,i.width,i.height).data,_=p.length/4|0,h=new Float32Array(_);for(let r=0,a=0;r<p.length;r+=4,a++){const m=p[r],O=p[r+1],y=p[r+2],c=(-1e4+(m*256*256+O*256+y)*.1)*t;h[a]=c}return{width:i.width,height:i.height,data:h}},Se=async()=>{const{data:e,width:t,height:o}=await Te("./terrainrgb.png",.15),u=1,s=1,i=new Kt,x=t*u/2,p=o*s/2,_=new Float32Array(t*o*3);for(let c=0;c<o;c++)for(let l=0;l<t;l++){const b=c*t+l,v=l*u-x,E=e[b],P=c*s-p,D=b*3;_[D]=v,_[D+1]=E,_[D+2]=P}i.setAttribute("position",new V(_,3));const h=new Float32Array(t*o*2);for(let c=0;c<o;c++)for(let l=0;l<t;l++){const b=c*t+l,v=l/(t-1),E=c/(o-1),P=b*2;h[P]=v,h[P+1]=E}i.setAttribute("uv",new V(h,2));const r=(t-1)*(o-1),a=new Uint32Array(r*6);let m=0;for(let c=0;c<o-1;c++)for(let l=0;l<t-1;l++){const b=c*t+l,v=b+t,E=b+1,P=v+1;a[m++]=b,a[m++]=v,a[m++]=E,a[m++]=v,a[m++]=P,a[m++]=E}i.setIndex(new V(a,1));const O=new xt(i,Ce);O.name="dem",i.computeVertexNormals();const y=new Gt().makeRotationY(Math.PI/-2);return i.applyMatrix4(y),O};var Oe=yt("<button>マップを見る</button>"),De=yt('<div class="fixed h-dvh w-full bg-gray-900"><canvas class="absolute m-0 block h-full w-full overflow-hidden bg-gray-900 p-0"></canvas> <div class="pointer-events-none absolute top-0 left-0 z-10 h-full w-full"><div class="flex h-full w-full flex-col items-center justify-center"><span class="c-text-shadow font-bold text-white max-lg:text-[75px] lg:text-[100px] svelte-1uha8ag">morivis</span> <!></div></div> <div class="absolute bottom-8 flex w-full items-center px-8 opacity-90 max-lg:justify-center lg:justify-between"><div class="flex gap-3 max-lg:hidden"><a class="pointer-events-auto flex cursor-pointer items-center text-white" href="https://github.com/forestacdev/morivis" target="_blank" rel="noopener noreferrer"><!></a> <button class="pointer-events-auto flex cursor-pointer items-center text-white"><!></button></div> <a class="pointer-events-auto shrink-0 cursor-pointer [&amp;_path]:fill-white" href="https://www.forest.ac.jp/" target="_blank" rel="noopener noreferrer"><!></a> <button class="pointer-events-auto flex shrink-0 cursor-pointer items-center p-2 text-white max-lg:hidden"><span class="underline select-none">利用規約</span></button></div></div>');function Ie(e,t){At(t,!0);const o=()=>H(lt,"$showInfoDialog",i),u=()=>H(Y,"$showTermsDialog",i),s=()=>H(Wt,"$isBlocked",i),[i,x]=Ft();let p=rt(null),_,h,r,a,m,O=rt(!0),y,c,l;const b=()=>{const d=window.location.hostname==="localhost";ht(O,!1),d?ft("/map"):ft("/morivis/map")},v=()=>{const d=window.innerWidth,f=window.innerHeight;G.resolution.value.set(d,f),r.setPixelRatio(window.devicePixelRatio),r.setSize(d,f),h.aspect=d/f,h.updateProjectionMatrix(),K.resolution.value.set(d,f),y&&(y.setSize(d,f),l.material.uniforms.resolution.value.set(d,f))};Nt(async()=>{if(!C(p)||!C(p))return;const d={width:window.innerWidth,height:window.innerHeight};_=new mt,h=new Qt(75,window.innerWidth/window.innerHeight,.1,1e5);const ot=-170*Math.PI/180,it=180;h.position.x=it*Math.sin(ot),h.position.z=it*Math.cos(ot),h.position.y=90,_.add(h),c=new mt;const Ot=C(p).getContext("webgl2");a=new Jt(h,C(p)),a.enableDamping=!0,a.enablePan=!1,a.enableZoom=!1,a.autoRotateSpeed=.5,a.autoRotate=!0,a.minDistance=100,a.maxDistance=500,a.maxPolarAngle=Math.PI/2-.35,m=new ae(h,C(p)),m.noPan=!0,m.noRotate=!0,m.zoomSpeed=.2,y=new te(d.width,d.height,{depthBuffer:!1,stencilBuffer:!1,magFilter:_t,minFilter:_t,wrapS:ut,wrapT:ut}),K.screenTexture.value=y.texture;const Dt=new ee(2,2),kt=new bt({fragmentShader:be,vertexShader:xe,uniforms:K});l=new xt(Dt,kt),c.add(l),r=new oe({canvas:C(p),context:Ot,alpha:!0}),r.setSize(window.innerWidth,window.innerHeight),r.setPixelRatio(Math.min(window.devicePixelRatio,2));const nt=await Se();nt?_.add(nt):console.error("Failed to create DEM mesh");const jt=new ie,st=()=>{requestAnimationFrame(st),a.update(),m.update(),G.time.value=jt.getElapsedTime(),r.setRenderTarget(y),r.render(_,h),r.setRenderTarget(null),r.render(c,h),l.material.uniforms.resolution.value.set(window.innerWidth,window.innerHeight)};st(),window.addEventListener("resize",v),v(),ne()||Y.set(!0)}),Lt(()=>{a.dispose(),m.dispose(),_.clear(),r.dispose(),y.dispose(),l.geometry.dispose(),window.removeEventListener("resize",v)});const E=()=>{lt.set(!o())},P=()=>{Y.set(!u())};var D=De(),J=M(D);Zt(J,d=>ht(p,d),()=>C(p));var I=k(J,2),tt=M(I),Et=k(M(tt),2);{var Pt=d=>{var f=Oe();f.__click=b,It(()=>{Ht(f,1,`bg-base text-main lg:hover:bg-main pointer-events-auto shrink-0 cursor-pointer rounded-full px-8 py-4 transition-all duration-200 max-lg:text-lg lg:text-2xl lg:hover:text-white ${s()?"pointer-events-none":"pointer-events-auto"}`),f.disabled=s()}),Yt(3,f,()=>Vt,()=>({duration:300,axis:"y"})),at(d,f)};Ut(Et,d=>{!s()&&C(O)&&d(Pt)})}T(tt),T(I);var et=k(I,2),Z=M(et),U=M(Z),Mt=M(U);ct(Mt,{icon:"mdi:github",class:"h-8 w-8"}),T(U);var F=k(U,2);F.__click=E;var Ct=M(F);ct(Ct,{icon:"akar-icons:info-fill",class:"h-7 w-7"}),T(F),T(Z);var W=k(Z,2),Tt=M(W);qt(Tt,{width:"230"}),T(W);var St=k(W,2);St.__click=P,T(et),T(D),at(e,D),Rt(),x()}zt(["click"]);export{Ie as component};
