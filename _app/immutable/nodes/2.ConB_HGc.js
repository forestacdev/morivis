...
}`,ft=e=>Math.pow(2,Math.ceil(Math.log2(e))),Pe=(e="morivis",t=1280,o="#ffffff",u="Arial, sans-serif")=>{const r=document.createElement("canvas"),i=r.getContext("2d");if(!i)throw new Error("Failed to get canvas context");i.font=`${t}px ${u}`,i.textAlign="left",i.textBaseline="top";const w=i.measureText(e),p=Math.ceil(w.width)+20,_=t+20,h=ft(p),a=ft(_);return r.width=h,r.height=a,i.clearRect(0,0,h,a),i.font=`${t}px ${u}`,i.textAlign="left",i.textBaseline="top",i.fillStyle=o,i.fillText(e,10,10),i.getImageData(0,0,r.width,r.height),r},K={time:{value:0},uColor:{value:new lt("rgb(252, 252, 252)")},uColor2:{value:new lt("rgb(0, 194, 36)")},resolution:{value:new v(window.innerWidth,window.innerHeight)}},G={screenCenter:{value:new v(.5,.5)},resolution:{value:new v(window.innerWidth,window.innerHeight)},screenTexture:{value:null},uTexture:{value:new Xt(Pe())},uTextureResolution:{value:new v(1e3,750)}},Me=new wt({uniforms:K,vertexShader:Ee,fragmentShader:xe,transparent:!0}),Ce=async(e,t=.1)=>{const u=await(await fetch(e)).blob(),r=new Image;r.src=URL.createObjectURL(u),await new Promise((a,s)=>{r.onload=a,r.onerror=s});const i=document.createElement("canvas");i.width=r.width,i.height=r.height;const w=i.getContext("2d");if(!w)throw new Error("Failed to get canvas context");w.drawImage(r,0,0);const p=w.getImageData(0,0,i.width,i.height).data,_=p.length/4|0,h=new Float32Array(_);for(let a=0,s=0;a<p.length;a+=4,s++){const d=p[a],O=p[a+1],b=p[a+2],c=(-1e4+(d*256*256+O*256+b)*.1)*t;h[s]=c}return{width:i.width,height:i.height,data:h}},Te=async()=>{const{data:e,width:t,height:o}=await Ce("./terrainrgb.png",.15),u=1,r=1,i=new Bt,w=t*u/2,p=o*r/2,_=new Float32Array(t*o*3);for(let c=0;c<o;c++)for(let l=0;l<t;l++){const g=c*t+l,E=l*u-w,P=e[g],x=c*r-p,D=g*3;_[D]=E,_[D+1]=P,_[D+2]=x}i.setAttribute("position",new V(_,3));const h=new Float32Array(t*o*2);for(let c=0;c<o;c++)for(let l=0;l<t;l++){const g=c*t+l,E=l/(t-1),P=c/(o-1),x=g*2;h[x]=E,h[x+1]=P}i.setAttribute("uv",new V(h,2));const a=(t-1)*(o-1),s=new Uint32Array(a*6);let d=0;for(let c=0;c<o-1;c++)for(let l=0;l<t-1;l++){const g=c*t+l,E=g+t,P=g+1,x=E+1;s[d++]=g,s[d++]=E,s[d++]=P,s[d++]=E,s[d++]=x,s[d++]=P}i.setIndex(new V(s,1));const O=new yt(i,Me);O.name="dem",i.computeVertexNormals();const b=new $t().makeRotationY(Math.PI/-2);return i.applyMatrix4(b),O},Se=(e,t)=>{gt(t,!1),oe("/morivis/map")};var Oe=vt("<button>マップを見る</button>"),De=vt('<div class="app relative flex h-dvh w-dvw"><canvas class="h-full w-full bg-gray-900"></canvas> <div class="pointer-events-none absolute left-0 top-0 z-10 h-full w-full"><div class="flex h-full w-full flex-col items-center justify-center"><span class="c-text-shadow font-bold text-white max-lg:text-[75px] lg:text-[100px] svelte-1xggvkh">morivis</span> <!></div></div> <div class="absolute bottom-8 flex w-full items-center px-8 opacity-90 max-lg:justify-center lg:justify-between"><div class="flex gap-3 max-lg:hidden"><a class="pointer-events-auto flex cursor-pointer items-center text-white" href="https://github.com/forestacdev/morivis" target="_blank" rel="noopener noreferrer"><!></a> <button class="pointer-events-auto flex cursor-pointer items-center text-white"><!></button></div> <a class="pointer-events-auto shrink-0 cursor-pointer [&amp;_path]:fill-white" href="https://www.forest.ac.jp/" target="_blank" rel="noopener noreferrer"><!></a> <button class="pointer-events-auto flex shrink-0 cursor-pointer items-center p-2 text-white max-lg:hidden"><span class="select-none underline">利用規約</span></button></div></div>');function Ie(e,t){jt(t,!0);const[o,u]=Zt(),r=()=>H(ht,"$showInfoDialog",o),i=()=>H(Y,"$showTermsDialog",o),w=()=>H(Ut,"$isBlocked",o);let p=at(null),_,h,a,s,d,O=at(!0),b,c,l;const g=()=>{const m=window.innerWidth,f=window.innerHeight;K.resolution.value.set(m,f),a.setPixelRatio(window.devicePixelRatio),a.setSize(m,f),h.aspect=m/f,h.updateProjectionMatrix(),G.resolution.value.set(m,f),b&&(b.setSize(m,f),l.material.uniforms.resolution.value.set(m,f))};zt(async()=>{if(!S(p)||!S(p))return;const m={width:window.innerWidth,height:window.innerHeight};_=new pt,h=new Kt(75,window.innerWidth/window.innerHeight,.1,1e5);const et=-170*Math.PI/180,ot=180;h.position.x=ot*Math.sin(et),h.position.z=ot*Math.cos(et),h.position.y=90,_.add(h),c=new pt;const Tt=S(p).getContext("webgl2");s=new qt(h,S(p)),s.enableDamping=!0,s.enablePan=!1,s.enableZoom=!1,s.autoRotateSpeed=.5,s.autoRotate=!0,s.minDistance=100,s.maxDistance=500,s.maxPolarAngle=Math.PI/2-.35,d=new se(h,S(p)),d.noPan=!0,d.noRotate=!0,d.zoomSpeed=.2,b=new Qt(m.width,m.height,{depthBuffer:!1,stencilBuffer:!1,magFilter:mt,minFilter:mt,wrapS:dt,wrapT:dt}),G.screenTexture.value=b.texture;const St=new Jt(2,2),Ot=new wt({fragmentShader:ye,vertexShader:be,uniforms:G});l=new yt(St,Ot),c.add(l),a=new te({canvas:S(p),context:Tt,alpha:!0}),a.setSize(window.innerWidth,window.innerHeight),a.setPixelRatio(Math.min(window.devicePixelRatio,2));const it=await Te();it?_.add(it):console.error("Failed to create DEM mesh");const Dt=new ee,nt=()=>{requestAnimationFrame(nt),s.update(),d.update(),K.time.value=Dt.getElapsedTime(),a.setRenderTarget(b),a.render(_,h),a.setRenderTarget(null),a.render(c,h),l.material.uniforms.resolution.value.set(window.innerWidth,window.innerHeight)};nt(),window.addEventListener("resize",g),g(),ie()||Y.set(!0)}),At(()=>{s.dispose(),d.dispose(),_.clear(),a.dispose(),b.dispose(),l.geometry.dispose(),window.removeEventListener("resize",g)});const E=()=>{ht.set(!r())},P=()=>{Y.set(!i())};var x=De(),D=M(x);Rt(D,m=>gt(p,m),()=>S(p));var I=k(D,2),J=M(I),bt=k(M(J),2);{var xt=m=>{var f=Oe();f.__click=[Se,O],Lt(()=>{Ft(f,1,`bg-base hover:bg-main pointer-events-auto shrink-0 cursor-pointer rounded-full px-8 py-4 transition-all duration-200 hover:text-white lg:text-2xl ${w()?"pointer-events-none":"pointer-events-auto"}`),f.disabled=w()}),Wt(3,f,()=>Ht,()=>({duration:300,axis:"y"})),st(m,f)};It(bt,m=>{w()||m(xt)})}C(J),C(I);var tt=k(I,2),Z=M(tt),U=M(Z),Et=M(U);rt(Et,{icon:"mdi:github",class:"h-8 w-8"}),C(U);var F=k(U,2);F.__click=E;var Pt=M(F);rt(Pt,{icon:"akar-icons:info-fill",class:"h-7 w-7"}),C(F),C(Z);var W=k(Z,2),Mt=M(W);Gt(Mt,{width:"230"}),C(W);var Ct=k(W,2);Ct.__click=P,C(tt),C(x),st(e,x),Nt(),u()}kt(["click"]);export{Ie as component};
