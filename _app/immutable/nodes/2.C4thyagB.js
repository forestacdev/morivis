...
}`,Pe=""+new URL("../assets/terrainrgb.BPhhet7p.webp",import.meta.url).href,Me=`varying vec2 vUv;// fragmentShaderに渡すためのvarying変数
varying vec3 vPosition;
uniform float uTime;
varying vec3 vNormal;
varying mat4 vModelMatrix;
...
}`,wt=e=>Math.pow(2,Math.ceil(Math.log2(e))),Ce=(e="morivis",t=1280,o="#ffffff",h="Arial, sans-serif")=>{const a=document.createElement("canvas"),i=a.getContext("2d");if(!i)throw new Error("Failed to get canvas context");i.font=`${t}px ${h}`,i.textAlign="left",i.textBaseline="top";const S=i.measureText(e),u=Math.ceil(S.width)+20,c=t+20,s=wt(u),p=wt(c);return a.width=s,a.height=p,i.clearRect(0,0,s,p),i.font=`${t}px ${h}`,i.textAlign="left",i.textBaseline="top",i.fillStyle=o,i.fillText(e,10,10),i.getImageData(0,0,a.width,a.height),a},G={time:{value:0},uColor:{value:new lt("rgb(252, 252, 252)")},uColor2:{value:new lt("rgb(0, 194, 36)")},resolution:{value:new g(window.innerWidth,window.innerHeight)}},K={screenCenter:{value:new g(.5,.5)},resolution:{value:new g(window.innerWidth,window.innerHeight)},screenTexture:{value:null},uTexture:{value:new Ht(Ce())},uTextureResolution:{value:new g(1e3,750)}},Te=new bt({uniforms:G,vertexShader:Me,fragmentShader:Ee,transparent:!0}),Se=async(e,t=.1)=>{const o=new Image;o.src=e,await new Promise((c,s)=>{o.onload=c,o.onerror=s});const h=document.createElement("canvas");h.width=o.width,h.height=o.height;const a=h.getContext("2d");if(!a)throw new Error("Failed to get canvas context");a.drawImage(o,0,0);const i=a.getImageData(0,0,h.width,h.height).data,S=i.length/4|0,u=new Float32Array(S);for(let c=0,s=0;c<i.length;c+=4,s++){const p=i[c],r=i[c+1],_=i[c+2],O=(-1e4+(p*256*256+r*256+_)*.1)*t;u[s]=O}return{width:h.width,height:h.height,data:u}},Oe=async()=>{const{data:e,width:t,height:o}=await Se(Pe,.15),h=1,a=1,i=new Yt,S=t*h/2,u=o*a/2,c=new Float32Array(t*o*3);for(let d=0;d<o;d++)for(let l=0;l<t;l++){const y=d*t+l,v=l*h-S,x=e[y],E=d*a-u,D=y*3;c[D]=v,c[D+1]=x,c[D+2]=E}i.setAttribute("position",new Y(c,3));const s=new Float32Array(t*o*2);for(let d=0;d<o;d++)for(let l=0;l<t;l++){const y=d*t+l,v=l/(t-1),x=d/(o-1),E=y*2;s[E]=v,s[E+1]=x}i.setAttribute("uv",new Y(s,2));const p=(t-1)*(o-1),r=new Uint32Array(p*6);let _=0;for(let d=0;d<o-1;d++)for(let l=0;l<t-1;l++){const y=d*t+l,v=y+t,x=y+1,E=v+1;r[_++]=y,r[_++]=v,r[_++]=x,r[_++]=v,r[_++]=E,r[_++]=x}i.setIndex(new Y(r,1));const O=new xt(i,Te);O.name="dem",i.computeVertexNormals();const b=new Vt().makeRotationY(Math.PI/-2);return i.applyMatrix4(b),O};var De=yt("<button>マップを見る</button>"),ke=yt('<div class="fixed h-dvh w-full bg-gray-900"><canvas class="absolute m-0 block h-full w-full overflow-hidden bg-gray-900 p-0"></canvas> <div class="pointer-events-none absolute top-0 left-0 z-10 h-full w-full"><div class="flex h-full w-full flex-col items-center justify-center"><span class="c-text-shadow font-bold text-white max-lg:text-[75px] lg:text-[100px] svelte-1uha8ag">morivis</span> <!></div></div> <div class="absolute bottom-8 flex w-full items-center px-8 opacity-90 max-lg:justify-center lg:justify-between"><div class="flex gap-3 max-lg:hidden"><a class="pointer-events-auto flex cursor-pointer items-center text-white" href="https://github.com/forestacdev/morivis" target="_blank" rel="noopener noreferrer"><!></a> <button class="pointer-events-auto flex cursor-pointer items-center text-white"><!></button></div> <a class="pointer-events-auto shrink-0 cursor-pointer [&amp;_path]:fill-white" href="https://www.forest.ac.jp/" target="_blank" rel="noopener noreferrer"><!></a> <button class="pointer-events-auto flex shrink-0 cursor-pointer items-center p-2 text-white max-lg:hidden"><span class="underline select-none">利用規約</span></button></div></div>');function Ue(e,t){At(t,!0);const o=()=>H(dt,"$showInfoDialog",i),h=()=>H(V,"$showTermsDialog",i),a=()=>H(qt,"$isBlocked",i),[i,S]=Ut();let u=rt(null),c,s,p,r,_,O=rt(!0),b,d,l;const y=()=>{const m=window.location.hostname==="localhost";ht(O,!1),m?ft("/map"):ft("/morivis/map")},v=()=>{const m=window.innerWidth,f=window.innerHeight;G.resolution.value.set(m,f),p.setPixelRatio(window.devicePixelRatio),p.setSize(m,f),s.aspect=m/f,s.updateProjectionMatrix(),K.resolution.value.set(m,f),b&&(b.setSize(m,f),l.material.uniforms.resolution.value.set(m,f))};Nt(async()=>{if(!M(u)||!M(u))return;const m={width:window.innerWidth,height:window.innerHeight};c=new mt,s=new Bt(75,window.innerWidth/window.innerHeight,.1,1e5);const ot=-170*Math.PI/180,it=180;s.position.x=it*Math.sin(ot),s.position.z=it*Math.cos(ot),s.position.y=90,c.add(s),d=new mt;const Ot=M(u).getContext("webgl2");r=new ie(s,M(u)),r.enableDamping=!0,r.enablePan=!1,r.enableZoom=!1,r.autoRotateSpeed=.5,r.autoRotate=!0,r.minDistance=100,r.maxDistance=500,r.maxPolarAngle=Math.PI/2-.35,_=new ae(s,M(u)),_.noPan=!0,_.noRotate=!0,_.zoomSpeed=.2,b=new Xt(m.width,m.height,{depthBuffer:!1,stencilBuffer:!1,magFilter:_t,minFilter:_t,wrapS:ut,wrapT:ut}),K.screenTexture.value=b.texture;const Dt=new $t(2,2),kt=new bt({fragmentShader:be,vertexShader:xe,uniforms:K});l=new xt(Dt,kt),d.add(l),p=new Kt({canvas:M(u),context:Ot,alpha:!0}),p.setSize(window.innerWidth,window.innerHeight),p.setPixelRatio(Math.min(window.devicePixelRatio,2));const nt=await Oe();nt?c.add(nt):console.error("Failed to create DEM mesh");const jt=new Gt,st=()=>{requestAnimationFrame(st),r.update(),_.update(),G.time.value=jt.getElapsedTime(),p.setRenderTarget(b),p.render(c,s),p.setRenderTarget(null),p.render(d,s),l.material.uniforms.resolution.value.set(window.innerWidth,window.innerHeight)};st(),window.addEventListener("resize",v),v(),ne()||V.set(!0)}),Rt(()=>{r.dispose(),_.dispose(),c.clear(),p.dispose(),b.dispose(),l.geometry.dispose(),window.removeEventListener("resize",v)});const x=()=>{dt.set(!o())},E=()=>{V.set(!h())};var D=ke(),J=P(D);ee(J,m=>ht(u,m),()=>M(u));var I=k(J,2),tt=P(I),Et=k(P(tt),2);{var Pt=m=>{var f=De();f.__click=y,It(()=>{Qt(f,1,`bg-base text-main lg:hover:bg-main pointer-events-auto shrink-0 cursor-pointer rounded-full px-8 py-4 transition-all duration-200 max-lg:text-lg lg:text-2xl lg:hover:text-white ${a()?"pointer-events-none":"pointer-events-auto"}`),f.disabled=a()}),Jt(3,f,()=>te,()=>({duration:300,axis:"y"})),at(m,f)};Zt(Et,m=>{!a()&&M(O)&&m(Pt)})}C(tt),C(I);var et=k(I,2),Z=P(et),U=P(Z),Mt=P(U);pt(Mt,{icon:"mdi:github",class:"h-8 w-8"}),C(U);var F=k(U,2);F.__click=x;var Ct=P(F);pt(Ct,{icon:"akar-icons:info-fill",class:"h-7 w-7"}),C(F),C(Z);var W=k(Z,2),Tt=P(W);oe(Tt,{width:"230"}),C(W);var St=k(W,2);St.__click=E,C(et),C(D),at(e,D),Lt(),S()}zt(["click"]);export{Ue as component};
