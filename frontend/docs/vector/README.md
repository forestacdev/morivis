[**morivis TypeDoc**](../README.md)

***

[morivis TypeDoc](../README.md) / vector

# vector

## Interfaces

- [AuxiliaryLayersData](interfaces/AuxiliaryLayersData.md)
- [FieldDef](interfaces/FieldDef.md)
- [LineStringEntry](interfaces/LineStringEntry.md)
- [PointEntry](interfaces/PointEntry.md)
- [PolygonEntry](interfaces/PolygonEntry.md)
- [TileMetaData](interfaces/TileMetaData.md)
- [Title](interfaces/Title.md)
- [VectorInteraction](interfaces/VectorInteraction.md)
- [VectorProperties](interfaces/VectorProperties.md)

## Type Aliases

- [GeoJsonMetaData](type-aliases/GeoJsonMetaData.md)
- [VectorEntry](type-aliases/VectorEntry.md)
- [VectorEntryGeometryType](type-aliases/VectorEntryGeometryType.md)
- [VectorFormatType](type-aliases/VectorFormatType.md)
