...
export const detectCrsCandidates = (...hints: string[]): string[] =>
	Array.from(new Set(hints.map((hint) => detectCrsInHint(hint)).filter((value) => value !== null)))
...
const collectLinkAttributes = (
	...
	let pending: { linkNumber: string; total: number; attributes: DrmLinkInnerAttribute[] } | null = null
		...
		if (pending === null) {
			...
			const properties: DrmLinkProperties = {
				...
				異常気象時通行規制区間種別コード: layout.weatherRestriction
					? readInt(record, layout.weatherRestriction.start, layout.weatherRestriction.length)
					: undefined,
				車両重量制限有無コード: layout.vehicleWeightLimit
					? readInt(record, layout.vehicleWeightLimit.start, layout.vehicleWeightLimit.length)
					: undefined,
				車両高さ制限有無コード: layout.vehicleHeightLimit
					? readInt(record, layout.vehicleHeightLimit.start, layout.vehicleHeightLimit.length)
					: undefined,
				車両幅制限有無コード: layout.vehicleWidthLimit
					? readInt(record, layout.vehicleWidthLimit.start, layout.vehicleWidthLimit.length)
					: undefined,
				...
				最小車道部幅員: layout.minCarriagewayWidth
					? readInt(record, layout.minCarriagewayWidth.start, layout.minCarriagewayWidth.length)
					: undefined,
				...
				交通規制種別コード: layout.trafficRegulationType
					? readInt(record, layout.trafficRegulationType.start, layout.trafficRegulationType.length)
					: undefined,
				...
				特車通行システム対象コード: layout.specialVehicleRoute
					? readInt(record, layout.specialVehicleRoute.start, layout.specialVehicleRoute.length)
					: undefined,
				リンク内属性有無コード: layout.linkAttributePresence
					? readInt(record, layout.linkAttributePresence.start, layout.linkAttributePresence.length)
					: undefined,
			...
			if (properties.対応基本道路ノード1 !== undefined && properties.対応基本道路ノード2 !== undefined) {
			...
			properties.リンク種別 = getCodeLabel(LINK_TYPE_LABELS, properties.リンク種別コード ?? null)
			...
			properties.規制速度 = getCodeLabel(SPEED_LIMIT_LABELS, properties.規制速度コード ?? null)
...
export const parseMesh = (
	...
	for (const recordId of recordIds) {
		...
		if (!layout) {
			throw new DrmError(
				`レコードID "${recordId}" のレイアウトは未定義です（対応: ${[...LAYOUTS.keys()].join(', ')}）。`
			)
		}
