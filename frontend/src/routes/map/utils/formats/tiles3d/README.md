...
- `local-files.ts` が相対パスとFileの対応を保持し、TilesRendererの取得プラグイン から必要なファイルだけ返す。
...
入力からの流れは `File[] → LocalTiles3DForm → Tiles3DEntry → Tiles3DLayerManager → TilesRenderer`。
Fileやfetch関数はentryに入れず、runtime側で管理する。
タイルの取得プラグインとglTFローダーが同じ取得関数を使い、外部buffer・画像にも適用する。
