// Adapted from the MIT-licensed `bz2` package by SheetJS.
...
const masks = [
	...
	0x00ffffff, 0x01ffffff, 0x03ffffff, 0x07ffffff, 0x0fffffff, 0x1fffffff, 0x3fffffff, -0x80000000
] as const;
...
interface HuffmanTable {
	table: HuffmanEntry[];
	fastAccess: Record<number, Record<number, HuffmanEntry>>;
}

const createOrderedHuffmanTable = (lengths: number[]): HuffmanTable => {
	...
	let tempBits = 0;
	let symbol = -1;
	const fastAccess: Record<number, Record<number, HuffmanEntry>> = {};
	let current: Record<number, HuffmanEntry> | undefined;

	for (const entry of table) {
		symbol += 1;
		if (entry.bits !== tempBits) {
			symbol <<= entry.bits - tempBits;
			tempBits = entry.bits;
			current = {};
			fastAccess[tempBits] = current;
		}
		entry.symbol = symbol;
		current![symbol] = entry;
	}

	return { table, fastAccess };
};

const bwtReverse = (src: number[], primary: number) => {
	...
	for (let j = 1; j < src.length; j += 1) {
		i = links[i];
		const value = src[i];
		output.push(value === undefined ? 255 : value);
	}
...
export const decompressBz2 = (bytes: Uint8Array, checkCRC = false): Uint8Array => {
	let index = 0;
	let bitfield = 0;
	let bits = 0;
	...
	const read = (n: number): number => {
		if (n >= 32) {
			const half = n >> 1;
			return read(half) * 2 ** half + read(n - half);
		}
		...
		const mask = masks[n];
		const result = (bitfield >> (bits - n)) & mask;
		bits -= n;
		bitfield &= ~(mask << bits);
		return result;
	};
	...
	while (true) {
		...
		if (blocktype === 0x314159265359) {
			...
			const symbolsInUse = used.reduce((sum, flag) => sum + Number(flag), 0) + 2;
			...
			while (true) {
				...
				if (decoded <= 0) {
					...
					if (selectorPointer <= selectors.length) {
						table = tables[selectors[selectorPointer]];
						selectorPointer += 1;
					}
				}

				for (const bitKey of Object.keys(table.fastAccess)) {
					const bitLength = Number(bitKey);
					if (bits < bitLength) {
						bitfield = (bitfield << 8) + bytes[index];
						index += 1;
						bits += 8;
					...
					const entry = table.fastAccess[bitLength][bitfield >> (bits - bitLength)];
					if (entry) {
						bitfield &= masks[(bits -= bitLength)];
						symbol = entry.code;
						break;
					}
				}
				...
				if (symbol >= 0 && symbol <= 1) {
					...
					repeat += repeatPower << symbol;
					repeatPower <<= 1;
					continue;
				}

				const favorite = favourites[0];
				for (; repeat > 0; repeat -= 1) {
					buffer.push(favorite);
				}
				...
				const value = favourites[symbol - 1];
				for (let j = symbol - 1; j > 0; j -= 1) {
			...
			const transformed = bwtReverse(buffer, pointer);
			let i = 0;
			while (i < transformed.length) {
				const charCode = transformed[i];
				let count = 1;

				if (
					i < transformed.length - 4 &&
					transformed[i + 1] === charCode &&
					transformed[i + 2] === charCode &&
					transformed[i + 3] === charCode
				) {
					count = transformed[i + 4] + 4;
					i += 5;
				} else {
					i += 1;
				}
				...
				for (let j = 0; j < count; j += 1) {
					if (checkCRC) {
						newCRC = (newCRC << 8) ^ crc32Table[((newCRC >> 24) ^ charCode) & 0xff];
					}
