...
export type PDFValueType = 'name' | 'string' | 'int' | 'real' | 'array' | 'dict' | 'bool' | 'null';
...
function parseMeasure(
	...
	const gptsVal = measureDict.get('lgit:GPTS') ?? measureDict.get('GPTS');
	...
	const lptsVal = measureDict.get('lgit:LPTS') ?? measureDict.get('LPTS');
	...
	for (let i = 0; i < nPoints; i++) {
		...
		const pixel = ((dfULX * (1 - lx) + dfLRX * lx) / mediaBoxWidth) * mediaBoxWidth;
		const line = ((dfULY * (1 - ly) + dfLRY * ly) / mediaBoxHeight) * mediaBoxHeight;
	...
	return {
		...
		gcps,
		neatline: neatlineCoords.length >= 4 ? { coordinates: neatlineCoords } : undefined,
		encoding: 'ISO32000'
	};
}
