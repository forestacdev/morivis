...
export interface Geometry {
  type: GeometryType;
  coordinates: any; // GeoJSON-style coordinates
}
...
function pdfToSRS(
  ...
  const py =
    opts.geoTransformValid !== false ? (1 - y / ph) * ph : (y / ph) * ph;
...
function tokenize(content: string): string[] {
  ...
  const re = /([+-]?(?:\d+\.?\d*|\.\d+))|(\/[^\s/<>()\[\]{}%]+)|(\((?:[^()\\]|\\.|\((?:[^()\\]|\\.)*\))*\))|(<[0-9A-Fa-f\s]*>)|(<<|>>)|(\[|\])|([a-zA-Z][a-zA-Z0-9*'"]*\*?)/g;
  const tokens: string[] = [];
  let match;
...
export function parseContentStream(
  ...
  function popNumbers(n: number): number[] {
    const result: number[] = [];
    for (let i = 0; i < n; i++) {
      const val = stack.length > 0 ? parseFloat(stack.shift()!) : 0;
      result.push(isNaN(val) ? 0 : val);
    }
    return result;
  }

  // Rebuild the stack from right: we keep operands in order, pop from end
  // Actually, PDF uses postfix: operands are pushed, operator pops them
  // So stack grows left to right, and we pop N items from the end
    ...
    switch (tok) {
      ...
      case "c": {
        ...
        if (args.length === 6) {
          ...
          const prevX =
            coords.length >= 2 ? coords[coords.length - 2] : x1;
          const prevY =
            coords.length >= 2 ? coords[coords.length - 1] : y1;
      ...
      case "v": {
        ...
        if (args.length === 4) {
          ...
          const prevX =
            coords.length >= 2 ? coords[coords.length - 2] : x2;
          const prevY =
            coords.length >= 2 ? coords[coords.length - 1] : y2;
...
function buildLineGeometry(
  ...
  for (let i = 0; i < coords.length; i += 2) {
    if (coords[i] === NEW_SUBPATH && coords[i + 1] === NEW_SUBPATH) {
      ...
      coords[i] === CLOSE_SUBPATH &&
      coords[i + 1] === CLOSE_SUBPATH
    ) {
      ...
      if (
        ...
        const [x, y] = pdfToSRS(currentLine[0][0], currentLine[0][1], {
          ...opts,
          geoTransform: undefined,
        });
        // re-transform from raw
        currentLine.push(currentLine[0]);
      }
    } else if (
      coords[i] === FILL_SUBPATH &&
      coords[i + 1] === FILL_SUBPATH
    ) {
      // skip
    } else {
