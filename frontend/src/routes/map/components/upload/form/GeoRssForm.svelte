<script lang="ts">
	...
	import {
		GeoRssParseError,
		geoRssFileToGeoJson,
		type GeoRssParseResult
	} from '$routes/map/utils/formats/georss';
	...
	let parseResult: GeoRssParseResult | null = null;
	let rawGeojson: FeatureCollection | null = null;
	let geometryTypeOptions = $state<{ key: string; name: string }[]>([]);
	let selectedGeometryType = $state<VectorEntryGeometryType | ''>('');

	const geoRssFile = $derived.by(() => {
		if (!dropFile) return null;
		return getFirstUploadFile(dropFile);
	});

	const entryName = $derived(geoRssFile?.name.replace(/\.[^.]+$/, '') ?? 'GeoRSSデータ');

	$effect(() => {
		if (geoRssFile) {
			isProcessing.set(true);
			geoRssFileToGeoJson(geoRssFile)
				.then((result) => {
					parseResult = result;
				...
				.catch((error) => {
					showNotification(
						error instanceof GeoRssParseError
							? error.message
							: 'GeoRSSファイルの読み込みに失敗しました',
						'error'
					);
					console.error(error);
				})
	...
	const processGeojson = async () => {
		...
		if (parseResult?.requiresManualCrsSelection || !bbox || !isBboxValid(bbox)) {
			pendingZoneGeoRefData = {
	...
	const convertAndCreateEntry = async (epsgCode: EpsgCode) => {
		...
		try {
			...
			if (!filtered || filtered.features.length === 0) {
				showNotification('GeoRSSファイルの変換に失敗しました', 'error');
				return;
			}
			...
			showNotification('GeoRSSファイルの変換中にエラーが発生しました', 'error');
			console.error(error);
		} finally {
			isProcessing.set(false);
		}
	...
	$effect(() => {
		if (zoneConfirmedEpsg && showDialogType === 'georss') {
...
<div class="flex shrink-0 items-center justify-between overflow-auto pb-4">
	<span class="text-2xl font-bold">GeoRSSファイルの登録</span>
</div>
