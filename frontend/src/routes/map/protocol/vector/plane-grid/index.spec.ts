...
import { planeGridProtocol, terminatePlaneGridWorker } from './index';
...
const tile = { url: 'plane_grid://tile/7/100000/0/0/0.pbf' };
...
describe('PlaneGridWorkerのライフサイクル', () => {
	it('同じタイルの同時要求でも応答先が混ざらない', async () => {
		const first = planeGridProtocol.request(tile, new AbortController());
		const second = planeGridProtocol.request(tile, new AbortController());
	...
	it('中止した要求の遅延応答は別の要求に届かない', async () => {
		...
		const first = planeGridProtocol.request(tile, controller);
	...
	it('非表示時は未完了の要求を終了し、再表示時は新しいWorkerを使う', async () => {
		const request = planeGridProtocol.request(tile, new AbortController());
		const rejection = expect(request).rejects.toMatchObject({ name: 'AbortError' });
		terminatePlaneGridWorker();
		await rejection;
		expect(workers[0].terminate).toHaveBeenCalledOnce();
		const next = planeGridProtocol.request(tile, new AbortController());
	...
	it('事前に中止された要求ではWorkerを作らない', async () => {
		const controller = new AbortController();
		controller.abort();
		await expect(planeGridProtocol.request(tile, controller)).rejects.toMatchObject({
			name: 'AbortError'
		});
