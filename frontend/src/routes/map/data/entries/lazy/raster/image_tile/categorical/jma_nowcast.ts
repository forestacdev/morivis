...
const config: JmaNowcastConfig = {
	...
	mapImage: `${MAP_IMAGE_BASE_PATH}/jma_nowcast.webp`
};

const applyNowcastCategoricalStyle = (entry: RasterImageEntry<RasterCategoricalStyle>) => {
	entry.style = {
...
const entry = applyNowcastCategoricalStyle(
...
const catalogItem: GeoDataEntryCatalogItem = {
	...
	loadEntry: async () =>
		applyNowcastCategoricalStyle(
			(await loadJmaNowcastRasterEntry(
				config
			)) as unknown as RasterImageEntry<RasterCategoricalStyle>
		)
};
